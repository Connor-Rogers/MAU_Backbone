"""
Tooling Definition and Execution Module
"""
...
# Tool detection regex pattern
TOOL_PATTERN = re.compile(
    r"(?:```(?:json)?\s*|`)?"  # optional triple fence (json) or single backtick
    r'\{\s*"name"\s*:\s*"([^"]+)"\s*,'
    r'\s*"arguments"\s*:\s*({.*?})\s*'
    r"\}(?:\s*```|`)?",  # optional closing fence/backtick
    re.DOTALL,
)
