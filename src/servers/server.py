...
async def get_node(node_id: str) -> str:
    ...
    return graph_res.to_json(node) if node else f"Node {node_id} not found."
...
async def graph_stats() -> str:
    ...
    return graph_res.to_json(stats)
...
async def get_central(measure: str = 'pagerank', top_n: int = 10) -> str:
    ...
    return graph_res.to_json(central)
